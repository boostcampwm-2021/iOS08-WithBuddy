//
//  CGFloatExtension.swift
//  WithBuddy
//
//  Created by Inwoo Park on 2021/11/14.
//

import UIKit

extension CGFloat {
    static let outsidePartInset = CGFloat(20)
    static let innerPartInset = CGFloat(10)
    
    static let outsideLeadingInset = CGFloat(20)
    static let outsideTrailingInset = CGFloat(-20)
    
    static let backgroudInnerLeadingInset = CGFloat(20)
    static let backgroudInnerTrailingInset = CGFloat(-20)
    static let backgroudInnerTopInset = CGFloat(20)
    static let backgroudInnerBottomInset = CGFloat(-20)
    static let backgroudHeight = CGFloat(45)
    
    static let purposeWidth = CGFloat(60)
    static let purposeHeight = CGFloat(90)
    static let purposeWholeHeight = CGFloat(200)
    
    static let buddyButtonInset = CGFloat(10)
    static let buddyWidth = CGFloat(60)
    static let buddyHeight = CGFloat(90)
    
    static let memoHeight = CGFloat(140)
    
    static let pictureAddButonSize = CGFloat(30)
    
    static let tableViewInset = CGFloat(10)
    static let tableViewHeight = CGFloat(150)
}
